...
void AutomationEditor::onDraw(Renderer& renderer, const SDL_Rect& area)
{
	...
	{
		renderer.renderLine(x, area.y, x, area.y + area.h, Theme::ColorType::AutomationVerticalLine);
	}
	...
	renderer.renderLine(time - getScrollPosition() + area.x, area.y, time - getScrollPosition() + area.x, area.y + area.h, Theme::ColorType::AutomationPlayHead);
	renderer.renderLine(editPos - getScrollPosition() + area.x, area.y, editPos - getScrollPosition() + area.x, area.y + area.h, Theme::ColorType::AutomationEditPos);
		...
		if (synth.getAutomationTrack(track).numNodes > 0)
		{
			...
			for (int i = 1 ; i < synth.getAutomationTrack(track).numNodes ; ++i)
			{
				...
				renderer.renderLine(px, py, x, y,
					...
					Theme::ColorType::AutomationSplineCurrent :
					Theme::ColorType::AutomationSpline);
	...
	if (getAutomationTrack().numNodes > 0)
	{
		...
		for (int i = 1 ; i < getAutomationTrack().numNodes ; ++i)
		{
			...
			renderer.renderLine(px, py, x, y, Theme::ColorType::AutomationSplineCurrent);
		...
		for (int i = 0 ; i < getAutomationTrack().numNodes ; ++i)
		{
			...
			renderer.clearRect(nodeArea, mSelectedNode == i ?
				Theme::ColorType::AutomationNodeSelected :
				Theme::ColorType::AutomationNode);
		}
	}
